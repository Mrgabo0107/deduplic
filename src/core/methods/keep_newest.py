...
def _build_synthetic_record_newest(
    rec_a: dict, 
    rec_b: dict, 
    node_a_id: int, 
    node_b_id: int
) -> dict:
    """Creates synthetic record C by selecting an exact copy of the newest record.
...
def keep_newest(
    ...
    """Applies the Keep-Newest deduplication strategy on a specific cluster edge."""
    ...
    # 1. Crear el registro sintético C seleccionando la copia ÍNTEGRA del más reciente
    synthetic_record = _build_synthetic_record_newest(rec_a, rec_b, node_a_id, node_b_id)
