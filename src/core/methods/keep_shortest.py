...
def _build_synthetic_record(
    ...
    the SHORTEST non-empty value for EVERY key present in either record."""
    synthetic_record = {}
    ...
    # 2. Para TODA clave, elegir el valor más corto (priorizando valores no vacíos)
    for key in clean_keys:
        ...
        # Si uno está vacío o es None, nos quedamos con el que sí tenga valor
        if not str_a:
            synthetic_record[key] = val_b
        elif not str_b:
            synthetic_record[key] = val_a
        else:
            # Ambos tienen contenido: elegimos el más corto
...
def keep_shortest(
    ...
    """Applies the Keep-Shortest deduplication strategy on a specific cluster edge."""
    ...
    # 1. Crear el registro sintético C evaluando la longitud MÍNIMA no vacía de TODAS las claves reales
