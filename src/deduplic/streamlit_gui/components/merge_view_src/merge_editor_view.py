    ...
    with col_left:
        ...
        with col_restore:
            if st.button("Restore", type="secondary", use_container_width=True):
                # RESTORE: Borra la edición personalizada para volver al texto base
        ...
        with col_cancel:
            if st.button("Cancel", type="secondary", use_container_width=True):
                # CANCEL: No modifica merge_status ni el archivo en disco,
                # sólo cierra el editor reteniendo lo que estuviera guardado previamente.
                st.session_state.pop("active_edit_key", None)
                st.session_state["open_merge_dialog"] = True
                st.rerun()
