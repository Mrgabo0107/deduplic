"""Deduplication and Merge Operational Services for Deduplic Streamlit GUI.

Handles edge/cluster resolutions, commits, restores, and merge manager actions.
"""
...
def delete_project_action(project_name: str) -> bool:
    """Elimina completamente un proyecto invocando el core."""
    project_path = _get_workspace_dir() / project_name
...
def _get_draft_paths(project_name: str) -> Tuple[Path, Path]:
    ...
    projects_dir = _get_workspace_dir()
...
def resolve_edge_action(project_name: str, component_id: int | str, edge_idx: int, method_name: str):
    """Resuelve una conexión específica entre dos registros dentro de un cluster."""
...
def resolve_cluster_action(project_name: str, component_id: int | str, method_name: str):
    """Resuelve un cluster completo aplicando un método determinado."""
    _get_draft_paths(project_name)
    project_path = _get_workspace_dir() / project_name
...
def resolve_all_action(project_name: str, method_name: str):
    """Aplica un método de deduplicación masivo a todos los clusters del proyecto."""
    _get_draft_paths(project_name)
    project_path = _get_workspace_dir() / project_name
...
def check_pending_merges_service(project_name: str) -> bool:
    """Verifica si existen borradores de merge sin resolver."""
    project_path = _get_workspace_dir() / project_name
    return deduplic_has_pending_merges(project_path)
...
def execute_single_merge_service(project_name: str, node_a_id: int, node_b_id: int, component_id: int | str):
    """Aplica el merge entre node_a y node_b y luego auto-limpia los borradores obsoletos."""
    project_path = _get_workspace_dir() / project_name
    
    result = deduplic_execute_merge(project_path, node_a_id, node_b_id, component_id)
...
def execute_all_merges_service(project_name: str, strategy: str = "Keep Most Complete") -> bool:
    """Procesa iterativamente todos los merges pendientes en lote."""
...
def forget_single_merge_service(project_name: str, filename: str):
    ...
    project_path = _get_workspace_dir() / project_name
    return deduplic_forget_single_merge(project_path, filename)


def get_pending_merges_service(project_name: str) -> List[Dict[str, Any]]:
    """Retorna la lista de merges pendientes actualizados con su estado actual."""
    project_path = _get_workspace_dir() / project_name
    return deduplic_list_pending_merges(project_path)


def forget_all_merges_service(project_name: str):
    """Elimina la carpeta completa de merges/ del proyecto."""
    project_path = _get_workspace_dir() / project_name
    deduplic_forget_merges(project_path, confirm=True)